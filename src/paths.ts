...
/**
 * daemon 的家目录。默认 ~/.asvc，可用 ASVC_HOME 覆盖。
 * 全局单实例：一台机器一个 daemon，便于跨项目去重、避免端口冲突。
 */
export const SVC_HOME = process.env.ASVC_HOME
  ? path.resolve(process.env.ASVC_HOME)
  : path.join(os.homedir(), ".asvc");

export const SOCKET_PATH =
  process.env.ASVC_SOCKET || path.join(SVC_HOME, "daemon.sock");
